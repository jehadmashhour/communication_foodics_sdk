...
actual class GrpcClientHandler actual constructor() {
    ...
    companion object { private const val TAG = "GrpcClient" }
    ...
    suspend fun connect(device: IoTDevice): Unit = withContext(Dispatchers.IO) {
        ...
        Log.i(TAG, "gRPC connected to ${device.name} @ ${device.address}")
    ...
    suspend fun sendToServer(data: ByteArray, writeType: WriteType): Unit = withContext(Dispatchers.IO) {
        ...
        runCatching { out.writeGrpcFrame(data) }.onFailure { Log.e(TAG, "Send error", it) }
    }
    ...
    fun disconnect() {
        ...
        Log.i(TAG, "gRPC client disconnected")
    }
    ...
    private suspend fun receiveLoop(sock: Socket) = withContext(Dispatchers.IO) {
        try {
            ...
            while (isActive && !sock.isClosed) {
                val frame = runCatching { input.readGrpcFrame() }.getOrNull() ?: break
