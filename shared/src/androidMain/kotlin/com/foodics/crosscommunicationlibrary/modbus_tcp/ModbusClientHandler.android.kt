...
actual class ModbusClientHandler actual constructor() {
    ...
    actual fun scan(): Flow<List<IoTDevice>> = channelFlow {
        val udp = withContext(Dispatchers.IO) {
            DatagramSocket(MODBUS_DISCOVERY_PORT).apply { soTimeout = 2_000; broadcast = true }
        }
        try {
            ...
            val evictJob = launch {
                while (isActive) {
                    ...
                    devices.entries.removeIf {
                        it.value.second.elapsedNow().inWholeMilliseconds > MODBUS_DEVICE_TTL_MS
                    }
            ...
            while (isActive) {
                ...
                try { withContext(Dispatchers.IO) { udp.receive(pkt) } }
                catch (_: java.net.SocketTimeoutException) { continue }
                catch (_: Exception) { break }

                val (name, id, port) = modbusParseBeacon(pkt.data.copyOf(pkt.length)) ?: continue
    ...
    actual fun sendToServer(data: ByteArray, writeType: WriteType) {
        val sock = socket ?: run { println("[ModbusClient] Not connected"); return }
        scope.launch(Dispatchers.IO) {
            runCatching { sock.getOutputStream().modbusWrite(buildModbusAdu(FC_UPLOAD, data)) }
                .onFailure { println("[ModbusClient] Send error: ${it.message}") }
        }
    }
