...
//    private var output: OutputStream? = null
    ...
    suspend fun start(deviceName: String, identifier: String) {
        ...
        // iOS needs some time to release previous service
        delay(1500)

        // 1️⃣ Start listening on TCP
//        serverSocket = ServerSocket(SERVICE_PORT)

        // 2️⃣ Create the NetService
        ...
        // 2️⃣ Register service with retry to avoid iOS timeout
...
//        Log.i(TAG, "LAN Server started on port $SERVICE_PORT and advertised via mDNS")
    ...
    /**
     * Retry registering service because iOS sometimes times out
     */
    private suspend fun registerService(service: NetService) {
        repeat(3) { attempt ->
            try {
