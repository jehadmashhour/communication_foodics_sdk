...
actual class TcpClientHandler {
    ...
    fun connect(device: IoTDevice) {
        ...
        val connFd = tcpConnect(ip, port)
        if (connFd < 0) { println("[TcpClient] Connect failed: errno=$errno"); return }
        fd = connFd
        println("[TcpClient] Connected to ${device.name} @ ${device.address}")
    ...
    fun sendToServer(data: ByteArray, writeType: WriteType) {
        val connFd = fd
        if (connFd < 0) { println("[TcpClient] Not connected"); return }
        tcpWriteFrame(connFd, data)
    }
    ...
    fun disconnect() {
        ...
        println("[TcpClient] Disconnected")
    }
    ...
    private suspend fun receiveLoop(connFd: Int) {
        while (scope.isActive && fd == connFd) {
            val frame = tcpReadFrame(connFd) ?: break
